...
const mp3s = ["fitz", "laser-01", "blarmp","splode"];
...
function getLaserBolt() {
  ...
  if (intersects.length > 0) {
    ...
    if (intersects[0].object.name === 'box') {
      ...
      fitzSound.stop();
      fitzSound.detune = Math.floor(Math.random() * 2400 - 800);
      fitzSound.play();
    }
  }
...
function fireLaser() {
  ...
  scene.add(laser);
  laserSound.stop();
  laserSound.detune = Math.floor(Math.random() * 1600 - 800);
  laserSound.play();
